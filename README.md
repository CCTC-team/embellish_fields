...
4. Injects this EM into `CCTC_REDCap_Docker/redcap_source/modules/embellish_fields_v1.0.3` and brings the Docker stack up (`app`, `db`, `mailhog`).
...
- `EM_NAME` / `EM_VERSION` — `embellish_fields` / `v1.0.3`. Bump `EM_VERSION` when releasing a new module version so the spec glob and inject path stay aligned.
